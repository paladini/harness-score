...
4. **行为导向。** 锁定实现细节的测试会阻碍合法重构；锁定行为的测试能捕获真实回归。[approved fixtures](https://lexler.github.io/augmented-coding-patterns/patterns/approved-fixtures/) 模式 — 经人工审查的黄金基准文件、由机器检查 — Böckeler 提到同事有选择地使用它，对智能体密集型代码库很有效。
...
Böckeler 的第二项调节维度是架构适切性 — 验证结构、而非仅语法的传感器：
