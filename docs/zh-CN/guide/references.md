...
- **[Harness engineering for coding agent users](https://martinfowler.com/articles/harness-engineering.html)** — martinfowler.com，2026 年 4 月。
  为*使用*智能体的团队框定该学科的文章：指南 vs 传感器、计算型 vs 推理型检查、三个调节维度（可维护性、架构 fitness、行为）、「质量左移」以及 harnessability 作为代码库属性。
- **[Harness Engineering — first thoughts](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering-memo.html)** — *Exploring Gen AI* 系列中术语成型的较早 memo。
- **[Improving Deep Agents with harness engineering](https://www.langchain.com/blog/improving-deep-agents-with-harness-engineering)** — LangChain，2026。
  实证案例：未动模型，Terminal Bench 2.0 上 52.8% → 66.5%。自验证循环、上下文组装 middleware、循环检测与推理三明治。
- **[deepagents](https://github.com/langchain-ai/deepagents)** — LangChain 开源「开箱即用智能体 harness」；可作为具体 harness 实现阅读。
...
- **[Agent Skills](https://cursor.com/docs/skills)** — SKILL.md 标准及 Cursor 如何加载 skills。
...
- 跨工具采用的 Agent Skills 开放标准 — skills 为 Cursor 编写即可移植的原因。
...
本仓库 dogfood 所教一切 — 自扫描得 L4，CI 以 `--min-level 4` 门禁。
