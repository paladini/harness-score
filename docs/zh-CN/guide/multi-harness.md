...
**maturity** 分数只统计仓库内文件 — 团队在 PR 中审查的内容。可选的 **effective**
在传递 `--scope user` 或在 [`.harness-score.json`](./measure-and-improve#scan-configuration)
中设置 `scopes.user` 时，可包含用户级安装。

| 位置 | 示例 | 计入 maturity | 计入 effective（启用时） |
|---|---|---|---|
| 仓库 | `.cursor/`、`AGENTS.md`、CI、测试 | 是 | 是 |
| 用户主目录 | `~/.cursor/…`、`~/.claude/…`、`~/.codeium/windsurf/…`、`~/Documents/Cline/Rules`、`~/.continue/…`、`~/.agents/…`、`~/.zed/…` | 否 | 是 |
| 共享 checkout | 指向团队 harness 的 `extraRoots` | 否 | 是 |

全局路径按工具 **allowlisted** — 扫描器不会遍历整个 `$HOME`。仅存在于 IDE
设置 UI 中的 Cursor User Rules 对两种分数均不可见。

### 各工具的 user scope 覆盖 {#user-scope-by-tool}

启用 `--scope user` 时，物理路径会映射为 registry 使用的相同 repo-relative
形状，使 checks 像仓库文件一样工作。
...
| **Zed** | `~/.zed/commands/` | |
| **GitHub Copilot** | — | 仅 repo：`.github/instructions/` |

**GitHub Copilot** 无已文档化的磁盘全局 instructions 路径 — 团队 instructions
应放在仓库中以保持 maturity/effective 一致。

完整 scope 表：[指标与代码](./metrics-and-codes#scopes)。
