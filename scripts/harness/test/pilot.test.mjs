...
test('toolkit integration joins the full gate only for an active pilot', (t) => {
  ...
  writeJson(paths(root).enabled, { version: '0.11.1' });
...
test('a timed-out active runtime denies acting events', (t) => {
  ...
  writeJson(join(paths(root).runtime, 'package.json'), { version: '0.11.1' });
  writeJson(paths(root).enabled, { version: '0.11.1' });
  let calls = 0;
  const execute = () => {
    calls++;
...
test('existing guard protects both provider payload shapes', () => {
  for (const claude of [false, true]) {
    for (const command of [
      ...
      const payload = claude ? { tool_input: { command } } : { command };
      const result = execFileSync(
        process.execPath,
        [join(ROOT, '.cursor/hooks/guard-shell.js'), ...(claude ? ['--claude'] : [])],
        { input: JSON.stringify(payload), encoding: 'utf8' },
      );
      const decision = JSON.parse(result);
      assert.equal(claude ? decision.hookSpecificOutput.permissionDecision : decision.permission, 'deny');
      if (!claude) assert.equal(typeof decision.user_message, 'string');
    }
  }
